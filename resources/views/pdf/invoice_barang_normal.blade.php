<table id="customers" style="padding-top: 20px;">
    ...
    <tr>
        ...
        <td class="text-right">{{number_format($val->det_quo_qty*$val->det_quo_harga_order,0, ',', '.')}}</td>
    </tr>
    @php
...
@if($kondisi=='partial' && ($inv->partial=='termin' or $inv->partial=='nominal'))
<table id="notes" style="width: 100%; padding-top: 20px;">
    ...
    <tbody>
        <tr>
            @php
            $percentage = $totalf*$inv->number/100;
            $tnominal = $inv->partial=='termin' ? $inv->number." %" : "";
            $nominal = $inv->partial=='termin' ? $percentage : $inv->number;
            @endphp
            <td class="text-center">Pembayaran partial dengan metode {{ucfirst($inv->partial.' '.$tnominal)}}</td>
